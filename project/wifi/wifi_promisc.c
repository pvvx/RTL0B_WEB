...
int _promisc_recv_func(_adapter *padapter, recv_frame *rframe);
#else
void _promisc_deinit(void *padapter);
int _promisc_recv_func(void *padapter, void *rframe);
#endif
int _promisc_set(rtw_rcr_level_t enabled, void (*callback)(uint8_t *, unsigned int, void *), uint8_t len_used);
int _is_promisc_enabled(void);
int _promisc_get_fixed_channel(void *fixed_bssid, uint8_t *ssid, int *ssid_length);
...
static u8 packet_filter_enable_num;
