...
#define SDK_PATCH 0
/* текущий режим WiFi */
unsigned char wifi_run_mode = RTW_MODE_NONE; // rtw_mode_t установленный режим работы
...
uint32_t read_wifi_cfg(uint32_t flg)
{
	uint32_t ret = 0;
...
uint32_t write_wifi_cfg(uint32_t flg)
{
	uint32_t ret = 0;
	PFEEP_ELEMENT p = feep_tab;
	for(int m = 1; m && p->id != 0; m <<= 1, p++) {
...
/* Определить и установить привязку drv WiFi к netif
 * Return: номер netif для AP */
LOCAL uint8_t chk_ap_netif_num(void)
{
...
rtw_result_t _wext_set_lps_dtim(int adapter_num, uint8_t lps_dtim ) {
...
rtw_result_t _wext_enable_powersave(int adapter_num, uint8_t ips_mode, uint8_t lps_mode) {
	_adapter * pad = get_padaptern(adapter_num);
	rtw_result_t ret = RTW_ERROR;
	if(pad) {
...
LOCAL rtw_result_t _wext_get_mode(int adapter_num, int *mode) {
	...
	rtw_result_t ret = RTW_ERROR;
	if(pad) {
		uint16_t f = pad->mlmepriv.fw_state;
		if(f & 8) *mode = 2;
...
LOCAL rtw_result_t _wext_get_channel(int adapter_num, uint8_t *ch)
{
	_adapter * pad = get_padaptern(adapter_num);
	rtw_result_t ret = RTW_ERROR;
	if(pad) {
...
extern int rltk_set_sta_num(int ap_sta_num);
...
*/
		if(rltk_set_sta_num(wifi_ap_cfg.max_sta) != 0) { // Max number of STAs, should be 1..3, default is 3
		...
		ret = _wifi_start_ap(wifi_ap_cfg.ssid,	//char  *ssid,
...
LOCAL void wifi_autoreconnect_thread_(void *param) {
	...
	warning_printf("auto reconnect ...\n");
	wifi_st_status = WIFI_STA_RECONNECT;
	if(wifi_st_cfg.autoreconnect == 255)
		wext_set_autoreconnect(WLAN0_NAME, 1, wifi_st_cfg.autoreconnect, wifi_st_cfg.reconnect_pause);
	ret = _wifi_connect(
			wifi_st_cfg.bssid,
			wifi_st_cfg.flg,
...
#endif
	xTaskCreate(wifi_autoreconnect_thread_, (const char * )"st_recon", 512,
			&wifi_autoreconnect, tskIDLE_PRIORITY + 1, NULL);
}

LOCAL int st_set_autoreconnect(uint8_t mode, uint8_t count, uint16_t timeout) {
	...
	return 0;
#else
	return wext_set_autoreconnect(WLAN0_NAME, mode, count, timeout);
#endif
}
...
//		p_wlan_autoreconnect_hdl = NULL;
		if (wifi_st_cfg.autoreconnect) {
			...
			if (ret != RTW_SUCCESS)
				warning_printf("Connection to AP failed! Error(%d)\n", ret);
		}
#endif
...
//		wifi_run_mode |= RTW_MODE_STA;
		if (ret != RTW_SUCCESS) {
			error_printf("Connection to AP failed! Error(%d)\n", ret);
		} else {
			if(wifi_cfg.save_flg & BID_WIFI_ST_CFG)
				write_wifi_cfg(BID_WIFI_ST_CFG);
			// Start DHCPClient
...
*/
	...
	uint8_t devnum = (mode == RTW_MODE_STA_AP); // flag = 1 -> 2 netif
	wifi_mode = mode;
	chk_ap_netif_num();
...
#endif
	if(wifi_mode != RTW_MODE_NONE) { // != mode) {
...
/*-- wifi tsf (64-bits counts, 1 us step) --*/
uint64_t wifi_get_tsf(void)
{
		if(wifi_mode <= RTW_MODE_STA_AP) return *((uint64_t *)(WIFI_REG_BASE + REG_TSFTR));
		else return 0;
}
