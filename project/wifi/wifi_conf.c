...
internal_join_result_t* join_user_data;
unsigned char wifi_mode = RTW_MODE_NONE; // rtw_mode_t режим драйвера
...
static int wifi_connect_local(rtw_network_info_t *pWifi) {
	...
	default:
		ret = -1;
		error_printf("WIFICONF: security type(0x%x) is not supported!\n",
				pWifi->security_type);
		break;
	}
	return ret;
...
void restore_wifi_info_to_flash() {
	...
	if (data_to_flash && p_write_reconnect_ptr) {
		if (wifi_get_setting((const char*) ifname[0], &setting)
			...
			warning_printf(" %s():wifi_get_setting fail or ap mode\n", __func__);
			return;
		}
...
int _wifi_start_ap(char *ssid, rtw_security_t security_type, char *password, int channel, char ssid_hidden) {
	const char *ifname = WLAN0_NAME;
	int ssid_len = 0;
	int password_len = 0;
	int ret = 0;
...
int _wifi_restart_ap(unsigned char *ssid, rtw_security_t security_type,
	...
	// start ap
	if (_wifi_start_ap((char*) ssid, security_type, (char*) password, channel, 0) < 0) {
	...
	if (idx > 0 && sta_linked == 0) {
		...
		printf("AP: key_idx =%d\n", setting.key_idx);
		ret = _wifi_connect(NULL, 0 , (char*) setting.ssid, setting.security_type,
...
extern int promisc_add_packet_filter(u8 filter_id,
		rtw_packet_filter_pattern_t *patt, rtw_packet_filter_rule_t rule);
