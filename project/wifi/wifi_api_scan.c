...
LOCAL int _wifi_scan_networks(rtw_scan_result_handler_t results_handler) {
	...
	pscan_rec->max_ap_size = MAX_AP_SCAN;
	pscan_rec->ap_details = (rtw_scan_result_t*) rtw_zmalloc(MAX_AP_SCAN * sizeof(rtw_scan_result_t) + MAX_AP_SCAN * sizeof(rtw_scan_result_t*));
	if (pscan_rec->ap_details != NULL) {
		pscan_rec->pap_details = (rtw_scan_result_t**) (&pscan_rec->ap_details[MAX_AP_SCAN]);
