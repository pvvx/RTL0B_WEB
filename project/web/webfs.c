...
#if 0
#define flash_mutex_lock() device_mutex_lock(RT_DEV_LOCK_FLASH)
#define flash_mutex_unlock() device_mutex_unlock(RT_DEV_LOCK_FLASH)
#else
#define flash_mutex_lock()
#define flash_mutex_unlock()
#endif

// Supports long file names to 64 characters
#define MAX_FILE_NAME_LEN   64 // VarNameSize
uint32_t disk_base_addr WEBFS_DATA_ATTR;
...
static uint32_t fatCacheID WEBFS_DATA_ATTR;
// Number of files in this WEBFS image
uint16_t numFiles WEBFS_DATA_ATTR;

LOCAL void GetFATRecord(uint16_t fatID);
...
/*****************************************************************************
  ...
  ***************************************************************************/
WEBFS_HANDLE WEBFS_CODE_ATTR WEBFSOpen(uint8_t* cFile)
{
	WEBFS_HANDLE hWEBFS;
	uint16_t nameHash;
	int i, len = 0; 
	uint16_t hashCache[16];
	uint8_t bufname[MAX_FILE_NAME_LEN];
	uint8_t *ptr;
	...
	for(i = 0; i < numFiles; i++) {
		...
		if((i & 0x0F) == 0)	{
			...
			WEBFSGetArray(0, (uint8_t*)hashCache, 32);
		}
...
/*****************************************************************************
  Function: uint16_t WEBFSGetArray(WEBFS_HANDLE hWEBFS, uint8_t* cData, uint16_t wLen)
  ...
  ***************************************************************************/
uint16_t WEBFS_CODE_ATTR  WEBFSGetArray(WEBFS_HANDLE hWEBFS, uint8_t* cData, uint16_t wLen)
{
	...
	// Read the data
		flash_mutex_lock();
		_flash_read(WEBFSStubs[hWEBFS].addr + WEBFS_HEAD_ADDR, wLen, cData);
		flash_mutex_unlock();
...
/*****************************************************************************
  ...
  ***************************************************************************/
bool WEBFS_CODE_ATTR WEBFSSeek(WEBFS_HANDLE hWEBFS, uint32_t dwOffset, WEBFS_SEEK_MODE tMode)
{
	uint32_t temp;
...
/*****************************************************************************
  Function:	static void GetFATRecord(uint16_t fatID)
  ...
  ***************************************************************************/
LOCAL void WEBFS_CODE_ATTR GetFATRecord(uint16_t fatID)
{
	WEBFS_FHEADER fhead;
	...
	WEBFSGetArray(0, (uint8_t *)&fatCache.data, 4);
	WEBFSStubs[0].addr = fatCache.data;
	WEBFSGetArray(0, (uint8_t *)&fhead, sizeof(fhead));
...
/*****************************************************************************
  Function:	uint16_t WEBFSGetFlags(WEBFS_HANDLE hWEBFS)
  ...
  ***************************************************************************/
uint16_t WEBFS_CODE_ATTR WEBFSGetFlags(WEBFS_HANDLE hWEBFS)
{
	// Make sure a valid file is open
	if(hWEBFS > MAX_WEBFS_OPENFILES	|| WEBFSStubs[hWEBFS].addr == WEBFS_INVALID) 
		return 0;
...
/*****************************************************************************
  ...
  ***************************************************************************/
uint32_t WEBFS_CODE_ATTR WEBFSGetSize(WEBFS_HANDLE hWEBFS)
{
	// Make sure a valid file is open
	if(hWEBFS > MAX_WEBFS_OPENFILES	|| WEBFSStubs[hWEBFS].addr == WEBFS_INVALID) 
		return 0;
...
/*****************************************************************************
  ...
  ***************************************************************************/
uint32_t WEBFS_CODE_ATTR WEBFSGetBytesRem(WEBFS_HANDLE hWEBFS)
{
	// Make sure a valid file is open
	if(hWEBFS > MAX_WEBFS_OPENFILES || WEBFSStubs[hWEBFS].addr == WEBFS_INVALID) 
		return 0;
...
/*****************************************************************************
  Function: bool WEBFSGetFilename(WEBFS_HANDLE hWEBFS, uint8* cName, uint16_t wLen)
  ...
  ***************************************************************************/
bool WEBFS_CODE_ATTR WEBFSGetFilename(WEBFS_HANDLE hWEBFS, uint8_t* cName, uint16_t wLen)
{
	uint32_t addr;
...
/*****************************************************************************
  ...
  ***************************************************************************/
uint32_t  WEBFS_CODE_ATTR WEBFSGetPosition(WEBFS_HANDLE hWEBFS)
{
...
LOCAL void WEBFS_CODE_ATTR WEBFS_Update(void)
{
	...
	WEBFSGetArray(0, (uint8_t*)&dhead, sizeof(dhead));
 ...
 * WEBFS_max_size()
 ***************************************************************************/
uint32_t WEBFS_CODE_ATTR WEBFS_max_size(void)
{
 ...
 * WEBFS_size()
 ***************************************************************************/
uint32_t WEBFS_CODE_ATTR WEBFS_curent_size(void)
{
	uint32_t size = 0;
	flash_mutex_lock();
	if(numFiles) size = _flash_read_dword(disk_base_addr + 8);
	flash_mutex_unlock();
	return size;
}
/****************************************************************************
 * WEBFS_size()
 ***************************************************************************/
uint32_t WEBFS_CODE_ATTR WEBFS_base_addr(void)
{
