...
#define OpenFlash()  device_mutex_lock(RT_DEV_LOCK_FLASH)
#define CloseFlash() device_mutex_unlock(RT_DEV_LOCK_FLASH)
...
void ICACHE_FLASH_ATTR web_test_adc(TCP_SERV_CONN *ts_conn)
{
    ...
    if(len > WAV_HEADER_SIZE + 10) {
    	...
    	len >>= 1;
    	read_adcs((uint16_t *)(web_conn->msgbuf + web_conn->msgbuflen), len, 0x0808);
...
void ICACHE_FLASH_ATTR web_modbus_xml(TCP_SERV_CONN *ts_conn)
{
	...
	while(web_conn->msgbuflen + 24 <= web_conn->msgbufsize) {
	    if(web_conn->udata_start < web_conn->udata_stop) {
	    	uint16_t val16;
	    	if(RdMdbData((uint8_t *)&val16, web_conn->udata_start, 1) != 0) tcp_puts_fd("<m%u>?</m%u>", web_conn->udata_start, web_conn->udata_start);
	    	else {
...
void ICACHE_FLASH_ATTR web_hexdump(TCP_SERV_CONN *ts_conn)
{
	...
	while(web_conn->msgbuflen + (9+3*16+17+2) <= web_conn->msgbufsize) {
			...
			tcp_puts("\r\n");
			if((uint32_t)addr >= web_conn->udata_stop) {
			    ClrSCB(SCB_RETRYCB);
			    if(!CheckSCB(SCB_WEBSOC)) SetSCB(SCB_FCLOSE | SCB_DISCONNECT); // connection close
			    return;
			}
...
//	device_mutex_lock(RT_DEV_LOCK_FLASH);
    _flash_read(web_conn->udata_start, len, web_conn->msgbuf);
...
//    	    SetSCB(SCB_FCLOSE | SCB_DISCONNECT);
    ...
    // Get/put as many bytes as possible
    uint32_t len = mMIN(web_conn->msgbufsize - web_conn->msgbuflen, web_conn->udata_stop - web_conn->udata_start);
/*
	if((uint32_t)web_conn->udata_start >= 0x98000000 && (uint32_t)web_conn->udata_start < 0x9A000000) {
		OpenFlash();
	}
...
*/
    copy_align4_to_align1(web_conn->msgbuf, (void *)(web_conn->udata_start), len);
	web_conn->msgbuflen += len;
	web_conn->udata_start += len;
#if DEBUGSOO > 2
	os_printf("%08x-%08x ",web_conn->udata_start, web_conn->udata_start + len );
 ...
 * Returns      : none
 ******************************************************************************/
void ICACHE_FLASH_ATTR web_int_callback(TCP_SERV_CONN *ts_conn, uint8_t *cstr)
{
    WEB_SRV_CONN *web_conn = (WEB_SRV_CONN *)ts_conn->linkd;
//        uint8_t *cstr = &web_conn->msgbuf[web_conn->msgbuflen];
        {
           uint8_t *vstr = strchr(cstr, '=');
           if(vstr != NULL) {
        	   *vstr++ = '\0';
        ...
        ifcmp("start") tcp_puts("0x%08x", web_conn->udata_start);
            ...
            ifcmp("scan") web_wscan_xml(ts_conn);
            	...
            	if(web_conn->auth_level < WEB_AUTH_LEVEL_USER) return;
            		...
            		ifcmp("ram") tcp_puts("0x%08x", *((uint32_t*)web_conn->udata_start));
            		else tcp_put('?');
          ...
          else ifcmp("cid") tcp_puts("%02x", 0xFF); //@TODO SYSCFG0_Get() HalGetChipId() ?
          else ifcmp("fid") tcp_puts("%08x", flash_get_id());
          else ifcmp("fsize") tcp_puts("%u", flash_get_size()); // flashchip->chip_size
...
//          else ifcmp("tcmh") tcp_puts("%u", tcm_heap_freeSpace());
        	  ...
        	  if(wifi_mode) {
        		  ...
        		  ux.dd = wifi_get_tsf(); // REG_TSFTR -> #include "hal_com_reg.h"
        		  tcp_puts("0x%08x%08x", ux.dw[1], ux.dw[0]);
        	  }
          }
          else ifcmp("clkcpu") tcp_puts("%u", CPU_ClkGet(0));
          else ifcmp("debug") tcp_put('1' - (ConfigDebugClose & 1)); // rtl_print on/off
#if WEB_DEBUG_FUNCTIONS
          else ifcmp("restart") {
#if USE_WEB_AUTH_LEVEL
        	  if(web_conn->auth_level < WEB_AUTH_LEVEL_USER) return;
          ...
          else ifcmp("ram") tcp_puts("0x%08x", *((uint32_t *)(ahextoul(cstr+3)&(~3))));
          else ifcmp("rdec") tcp_puts("%d", *((uint32_t *)(ahextoul(cstr+4)&(~3))));
#endif // #if WEB_DEBUG_FUNCTIONS
          else ifcmp("ip") {
        	  uint32_t cur_ip = 0;
...
extern int mlx90614_ws(TCP_SERV_CONN *ts_conn, char cmd);
        	  ...
        	  if(CheckSCB(SCB_WEBSOC)) {
extern int adc_ws(TCP_SERV_CONN *ts_conn, uint8_t cmd);
          ...
          else ifcmp("newcfg") webserver_qfn((web_ex_func_cb)wifi_run, (void *)((uint32_t)wifi_cfg.mode), 200);
    ...
    //    	  else ifcmp("eip") 	tcp_puts(IPSTR, IP2STR(&wifi_ap_dhcp.end_ip));
        	...
        	ifcmp("flash") {
        		...
        		if(*cstr == '_') {
            		...
            		ifcmp("all") {
            	    	web_conn->udata_start = 0;
            	    	web_conn->udata_stop = flash_get_size();
...
#endif
#ifdef USE_SNTP
		else ifcmp("sntp_time") tcp_puts("%u", (uint32_t) sntp_gen_system_time(0)); // get_sntp_time
#endif
#ifdef TEST_SEND_WAVE
