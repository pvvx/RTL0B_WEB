...
#include "web/web_utils.h" // base64encode()
...
const uint8_t WebSocketHTTPOkKey[] ICACHE_RODATA_ATTR = "HTTP/1.1 101 Switching Protocols\r\nAccess-Control-Allow-Origin: *\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: %s\r\n\r\n";
const uint8_t WebSocketAddKey[] ICACHE_RODATA_ATTR = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
...
bool ICACHE_FLASH_ATTR WebSocketAcceptKey(uint8_t* dkey, uint8_t* skey)
{
	int len = 0;
	bool ret = false;
...
WebsocketMask(WS_FRSTAT *ws, uint8_t *raw_data, uint32_t raw_len)
{
...
WebsocketHead(WS_FRSTAT *ws, uint8_t *raw_data, uint32_t raw_len)
{
...
WebsocketTxFrame(TCP_SERV_CONN *ts_conn, uint32_t opcode, uint8_t *raw_data, uint32_t raw_len)
{
	union {
		uint8_t uc[8];
		uint16_t uw[4];
