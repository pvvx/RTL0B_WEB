...
typedef uint32_t (* call_func)(uint32_t a, uint32_t b, uint32_t c);
 ...
 * Returns      : none
*******************************************************************************/
void ICACHE_FLASH_ATTR web_int_vars(TCP_SERV_CONN *ts_conn, uint8_t *pcmd, uint8_t *pvar)
{
    WEB_SRV_CONN *web_conn = (WEB_SRV_CONN *)ts_conn->linkd;
	uint32_t val = ahextoul(pvar);
	char *cstr = pcmd;
#if DEBUGSOO > 1
    os_printf("[%s=%s]\n", pcmd, pvar);
#endif
	ifcmp("start") 		web_conn->udata_start = val;
		...
		else ifcmp("ram") { uint32_t *ptr = (uint32_t *)(ahextoul(cstr+3)&(~3)); str_array(pvar, ptr, 32); }
		else ifcmp("debug") ConfigDebugClose = val == 0; // rtl_print on/off
		...
		ifcmp("web_") {
			...
			ifcmp("port") {
				if(syscfg.web_port != val) {
					webserver_qfn((web_ex_func_cb)webserver_reinit, (void *)((uint32_t)syscfg.web_port), 200);
					syscfg.web_port = val;
				}
			}
			else ifcmp("twd") {
...
*/
			...
			ConfigDebugClose = val == 0; // rtl_print on/off
		}
		else ifcmp("save") {
...
//      }
          ...
          ifcmp("ssid") {
        	  if(pvar[0]!='\0') {
        		  ...
        		  memset(wifi_ap_cfg.ssid, 0, sizeof(wifi_ap_cfg.ssid));
        		  memcpy(wifi_ap_cfg.ssid, pvar, len);
#ifdef USE_NETBIOS
//        		  netbios_set_name(wlan_ap_netifn, wifi_ap_cfg.ssid);
    		  ...
    		  memset(wifi_ap_cfg.password, 0, sizeof(wifi_ap_cfg.password));
    		  memcpy(wifi_ap_cfg.password, pvar, len);
          }
       		  ...
       		  if(len) {
       			  memset(lwip_host_name[1], 0, LWIP_NETIF_HOSTNAME_SIZE);
       			  memcpy(lwip_host_name[1], pvar, len);
       		  }
...
//    	  else ifcmp("eip") 	wifi_ap_dhcp.end_ip = ipaddr_addr(pvar);
          ...
          ifcmp("arec") 	wifi_st_cfg.autoreconnect = val;
        	  ...
        	  if(pvar[0]!='\0') {
           		  ...
           		  memset(wifi_st_cfg.ssid, 0, sizeof(wifi_st_cfg.ssid));
           		  memcpy(wifi_st_cfg.ssid, pvar, len);
        	  }
          }
          else ifcmp("psw") {
    		  int len = strlen(pvar);
    		  ...
    		  memset(wifi_st_cfg.password, 0, sizeof(wifi_st_cfg.password));
    		  memcpy(wifi_st_cfg.password, pvar, len);
          }
       		  ...
       		  if(len) {
       			  memset(lwip_host_name[0], 0, LWIP_NETIF_HOSTNAME_SIZE);
       			  memcpy(lwip_host_name[0], pvar, len);
