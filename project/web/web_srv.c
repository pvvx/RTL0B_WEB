...
LOCAL void webserver_file_ext(HTTP_CONN *CurHTTP, uint8_t *pfname) ICACHE_FLASH_ATTR;
...
LOCAL uint8_t ICACHE_FLASH_ATTR CheckAuthorization(uint8_t* base64str)
{
...
LOCAL void ICACHE_FLASH_ATTR web_parse_vars(TCP_SERV_CONN *ts_conn, uint8_t *vars, uint32_t vars_len, uint8_t start_char, uint8_t end_char)
{
...
//=============================================================================
//=============================================================================
uint8_t * ICACHE_FLASH_ATTR head_find_ctr(HTTP_CONN *CurHTTP, const uint8_t * c, int clen, int dlen)
{
...
*******************************************************************************/
bool ICACHE_FLASH_ATTR web_inc_fopen(TCP_SERV_CONN *ts_conn, uint8_t *cFile)
{
	WEB_SRV_CONN *web_conn = (WEB_SRV_CONN *)ts_conn->linkd;
 ...
 * Returns      : 1 - open, 0 - no
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR webserver_file_ext(HTTP_CONN *CurHTTP, uint8_t *pfname)
{
	uint8_t *pfext = NULL;
...
{
	WEB_SRV_CONN *web_conn = (WEB_SRV_CONN *)ts_conn->linkd;
	uint8_t pbuf[MAX_FILE_NAME_SIZE];
	uint8_t *pstr = pbuf;
...
/******************************************************************************
*******************************************************************************/
LOCAL int ICACHE_FLASH_ATTR web_find_cbs(uint8_t * chrbuf, uint32_t len) {
  uint32_t i;
	...
	if(CheckSCB(SCB_FCALBACK) == 0) { // передача файла без парсинга
		...
		do { // начинаем с пустого буфера
			if(CheckSCB(SCB_RETRYCB)) { // повторный callback? да
				...
				uint32_t len = WEBFSGetArray(web_conn->webfile, pstr, max);
...
LOCAL int ICACHE_FLASH_ATTR find_boundary(HTTP_UPLOAD *pupload, uint8_t *pstr, uint32_t len)
{
...
LOCAL int ICACHE_FLASH_ATTR upload_boundary(TCP_SERV_CONN *ts_conn) // HTTP_UPLOAD pupload, uint8_t pstr, uint16_t len)
{
...
//--- web_trim_bufi -----------------------------------------------------------
//-----------------------------------------------------------------------------
bool ICACHE_FLASH_ATTR web_trim_bufi(TCP_SERV_CONN *ts_conn, uint8_t *pdata, uint32_t data_len)
{
 ...
 * Returns      : none
*******************************************************************************/
err_t ICACHE_FLASH_ATTR webserver_init(uint16_t portn)
{
 ...
 * Returns      : none
*******************************************************************************/
err_t ICACHE_FLASH_ATTR webserver_close(uint16_t portn)
{
	err_t err = ERR_ARG;
 ...
 * Returns      : none
*******************************************************************************/
err_t ICACHE_FLASH_ATTR webserver_reinit(uint16_t portn)
{
	err_t err = ERR_OK;
